...
function addressUtils_isContract (const self : state; const addr : address) : (bool) is
  block {
...
function fallback_ (const self : state) : (state) is
  block {
    willFallback_(self);
    delegate_(self, implementation_(self));
  } with (self);

function implementation_ (const self : state) : (address) is
  block {
    skip
  } with ();

function delegate_ (const self : state; const implementation : address) : (state) is
  block {
    (* InlineAssembly {
...
function setImplementation_ (const self : state; const newImplementation : address) : (list(operation) * state) is
  block {
...
function upgradeTo_ (const self : state; const newImplementation : address) : (state) is
  block {
...
function upgradeabilityProxy_constructor (const self : state; const implementation_ : address; const data_ : bytes) : (list(operation) * state) is
  block {
    assert((self.IMPLEMENTATION_SLOT = sha_256("org.zeppelinos.proxy.implementation")));
    setImplementation_(self, implementation_);
    if (size(data_) > 0n) then block {
