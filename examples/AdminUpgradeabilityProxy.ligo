...
function implementation_ (const self : state_Proxy) : () is
  block {
    skip
  } with ();
...
function admin (const self : state; const receiver : contract(address)) : (list(operation)) is
  block {
...
function implementation (const self : state; const receiver : contract(address)) : (list(operation)) is
  block {
    if (sender = admin_(self)) then block {
      skip
    } else block {
