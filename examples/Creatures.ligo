...
type getCreature_args is record
  receiver : contract((address * nat * nat * nat * nat));
  id : nat;
end;
...
function constructor (const self : state_; const receiver : contract()) : () is
  block {
...
function getCaller (const self : state_Permissions; const receiver : contract(address)) : (list(operation)) is
  block {
    var opList : list(operation) := list transaction((self.callerAddress), 0mutez, receiver) end;
  } with (opList);

function getStorageAddress (const self : state_Permissions; const receiver : contract(address)) : (list(operation)) is
  block {
    var opList : list(operation) := list transaction((self.storageAddress), 0mutez, receiver) end;
  } with (opList);

function getOwner (const self : state_Permissions; const receiver : contract(address)) : (list(operation)) is
  block {
    var opList : list(operation) := list transaction((self.ownerAddress), 0mutez, receiver) end;
  } with (opList);
...
function getCreature (const self : state; const receiver : contract((address * nat * nat * nat * nat)); const id : nat) : (list(operation)) is
  block {
    ...
    var opList : list(operation) := list transaction(((owner, c.species, c.subSpecies, c.eyeColor, c.timestamp)), 0mutez, receiver) end;
  } with (opList);
